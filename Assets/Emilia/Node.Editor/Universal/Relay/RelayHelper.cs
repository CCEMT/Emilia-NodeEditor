...
namespace Emilia.Node.Universal.Editor
{
    ...
    public class DefaultRelayEdgeDataStrategy : IRelayEdgeDataStrategy
    {
    ...
    public static class RelayHelper
    {
        ...
        private static readonly Vector2 RelaySize = new(50, 20);
            ...
            where TRelayNodeAsset : RelayNodeAsset
        {
            ...
            relayAsset.position = new Rect(position, RelaySize);
            ...
            where TRelayNodeAsset : RelayNodeAsset
        {
            TRelayNodeAsset relayAsset = ScriptableObject.CreateInstance<TRelayNodeAsset>();
            relayAsset.id = Guid.NewGuid().ToString();
            relayAsset.position = new Rect(Vector2.zero, RelaySize);
            relayAsset.portOrientation = edgeView.outputPortView?.info.orientation ?? EditorOrientation.Horizontal;
        ...
        private static void InsertRelayGroup<TRelayNodeAsset>(
            ...
            where TRelayNodeAsset : RelayNodeAsset
        {
            EditorOrientation portOrientation = group.outputPorts.FirstOrDefault()?.info.orientation ?? EditorOrientation.Horizontal;
            ...
            RelayInsertResult result)
        {
            ...
            for (int i = 0; i < edgeCount; i++)
            {
                ...
                if (edgeView != null)
                {
                    result.createdEdgeViews.Add(edgeView);
                    edgeView.ForceUpdateView();
                }

                Undo.RegisterCreatedObjectUndo(edgeAssets[i], "Insert Relay Node");
            }
        }
